...
class Customers(Frame):
    ...
    def __init__(self, parent, controller):
        ...
        head = Label(func_frame, text='Customers', font=medium_lined, bg=back, fg='dark blue')
        ...
        dealb = Button(func_frame, relief=GROOVE, bd=4, text='Dealers', bg='red', fg='white', font=tiny_font, height=1,
                       width=10, command=lambda: controller.show_frame('Dealers'))
        dealb.grid(row=2, column=2)

        receivables = Label(func_frame, font=medium_font, bg='#52c433', fg='white', padx=25, pady=10,
                            relief=RIDGE, bd=5, width=10)
        receivables.grid(row=1, column=5, sticky=NE, padx=5)

        r_text = Label(func_frame, text='Receivables:',fg='purple', font=head_text, bg=back)
        ...
        def get_details_delete():
            ...
            Label(delete_window, text='Enter Customer Details:', font=small_font,bg=bg).grid(row=1, column=1,columnspan=2, sticky='nsew')
            ...
            delete_b = Button(delete_window,font=small_font, text='Delete',bg='orange',command=lambda:delete_customer(), width=2, height=1)
            delete_b.grid(row=4, column=2,sticky='nsew',padx=10, pady=10)

            def delete_customer():
                n = name.get()
                ad = address.get()
                delete_someone(n,ad,'Customer')
        ...
        def get_details_add():
            ...
            Label(add_window, text='Enter Customer Details:', font=small_font,bg=bg).grid(row=1, column=1, columnspan=2,sticky=NSEW)
            ...
            add_b = Button(add_window, text='Add',font=small_font,bg='#aeff2b', command=lambda :add_customer())
...
# --------------------show info function-----------------#
        ...
        def show_info(event):
            for selected_item in custs.selection():
                ...
                name = someone[0]
                address = someone[1]
                info =evaluate(name, address,'Customer')
                try:
                    balance = info[1]-info[2]
                except:
                    balance='NA'
                try:
                    contact = details(name,address,'Customer')
                except:
                ...
                Label(head_frame, text='All Transactions of '+name, font=huge_tree,bg=bg, fg='red', bd=4,relief=GROOVE).grid(row=1, column=1, columnspan=10,sticky='nsew', padx=10, pady=10)
                Label(head_frame,text='Phone:'+str(contact), font=dialog_font,bg=bg, fg='dark blue').grid(row=2, column=1,sticky='nw', padx=10, pady=10)
                Label(head_frame, text='Address:'+str(address), font=dialog_font, bg=bg, fg='dark blue').grid(row=3, sticky='nw', column=1, padx=10, pady=10)
                Label(head_frame, text='Balance:', font=dialog_font,bg=bg, fg='dark blue').grid(row=4 ,column=1,sticky='nw')
                Label(head_frame, text='Rs.'+str(balance), font=huge_tree,bd=5, bg='#00ff88', fg='white',relief=GROOVE).grid(row=4,column=2,sticky='nw', padx=10, pady=10)
                ...
                def refresh_info():
                    ...
                    transactions = get_all_for(name,address, 'Customer')
...
# ------------------------------------show table function ------------#
        ...
        custs.tag_configure('due', background='#ff9757')
        custs.tag_configure('unknown', background='#d5ccff')

        def refresh_and_add_all():
            custs.delete(*custs.get_children())
            customers = all_customers_dealers_with_address('Customer')
            for i in range(len(customers)):
                info = evaluate(customers[i][0],customers[i][1], 'Customer')
                name=customers[i][0]
                address=info[0]
                debit=info[1]
                credit=info[2]
            ...
            to_receive = receivables_payables('Customer')
            receivables.config(text='Rs.' + str(to_receive))
