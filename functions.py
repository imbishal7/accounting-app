...
class Dealer:
    ...
    def add(self):
        with conn:
            c.execute("INSERT INTO dealers VALUES(:name, :address, :contact)",{'name':self.name, 'address':self.address, 'contact':self.contact})
...
def get_all():
    c.execute("SELECT date, time, ref, name, type, remarks, debit, credit FROM statements")
    all = c.fetchall()
...
def get_all_for(name,address, type):
    c.execute("SELECT date, time, ref, name,remarks, debit, credit FROM statements WHERE name =:name AND address=:address AND type=:type", {'name':name, 'type':type,'address':address})
    a = c.fetchall()
...
def all_customers_dealers_with_address(sm):
    result =c.execute('SELECT name, address FROM statements WHERE type=:x', {'x': sm}).fetchall()
    if sm=='Customer':
        all_c = c.execute('SELECT name,address FROM customers').fetchall()
        all = [result[i] for i in range(len(result))]
        allc = [all_c[i] for i in range(len(all_c))]
    elif sm=='Dealer':
        all_c = c.execute('SELECT name,address FROM dealers').fetchall()
        all = [result[i] for i in range(len(result))]
        allc = [all_c[i] for i in range(len(all_c))]

    return list(set(all+allc))
